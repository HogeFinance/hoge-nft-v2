...
contract('HogeNFT', function ([ creator, other, holder ]) {
    ...
    it('minter can mint', async function () {
        ...
        expect(await this.token.tokenURI(tokenIdZero)).to.be.equal(`${baseURI}${someURI}`);
    ...
    it('mints batches', async function() {
        ...
        const receipt = await this.token.mintBatch(recipients, uris, { from: creator });
    ...
    it ('correctly assigns uris', async function() {
        ...
        const receipt  = await this.token.mintBatch([holder], [uri1], { from: creator }); // Token 0
        const receipt2 = await this.token.mintBatch([holder], [uri2], { from: creator }); // Token 1

        expect(await this.token.tokenURI(new BN('0'))).to.be.equal(baseURI + web3.utils.hexToAscii(uri1));
        expect(await this.token.tokenURI(new BN('1'))).to.be.equal(baseURI + web3.utils.hexToAscii(uri2));
    });
